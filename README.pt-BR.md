...
[![Calculadora Online](https://img.shields.io/badge/calculadora-online-success.svg?style=flat-square)](https://sandrobenigno.github.io/FOVCalc/)
[![Tecnologias](https://img.shields.io/badge/tecnologias-HTML5%20%7C%20CSS3%20%7C%20Three.js-blue.svg?style=flat-square)](#)
[![Licença: GPL v3](https://img.shields.io/badge/licenca-GNU%20GPL%20v3-blue.svg?style=flat-square)](LICENSE)
...
O primeiro ponto que precisamos ter em mente é a compreensão do que é distância focal. Ela é a distância física entre o centro óptico da lente e a superfície do sensor.
...
* **Fator de Corte** é o número que indica quantas vezes seu sensor é menor que o Full Frame.
...
> [!NOTE]
> Todas as fórmulas poderiam ser utilizadas também para calcular o ângulo de abertura vertical (VFOV). Bastaria substituir $x$ por $y$, mantendo $z$, onde $y$ seria a metade da altura do sensor, em vez da largura.
> 
> Mas atenção a um erro comum: você não deve calcular o ângulo vertical aplicando a proporção do aspecto direto no ângulo horizontal (por exemplo, achar que o VFOV é 3/4 do HFOV em um sensor 4:3). Como a relação envolve trigonometria, os ants não mudam de forma linear.
> 
> Por outro lado, se você aplicar a proporção do aspecto diretamente no tamanho físico da cena capturada, a regra funciona perfeitamente! Em um sensor 4:3, a altura real da cena será exatamente 3/4 da largura. E é justamente esse cálculo da largura física da cena que nos leva ao nosso próximo conceito essencial para o set: o HFW.
...
* **Cateto Adjacente**: A distância ($D$) da câmera até o objeto (a partir do centro óptico da lente).
* **Cateto Oposto**: A metade da largura da cena ($HFW/2$).
...
* **Passo 1: Calcular o HFOV**: Se o valor de distância focal dado é de 50mm em equivalência a 35mm, a conversão para valores em Full Frame já foi feita. Portanto, a largura do sensor será 36mm. Lembre-se: um filme de 35mm tem a largura do fotograma igual a 36mm. Nós utilizamos a metade do sensor para o cálculo, ou seja, 18mm. Assim, teremos:

$$
\text{HFOV} = 2 \times \arctan\left( \frac{18}{50} \right) \to 2 \times 19,79° = 39,6°
$$

* **Passo 2: Obter o semiângulo**: Tomaremos a metade do HFOV (exatamente o semiângulo 19,79° encontrado no meio do cálculo anterior):

$$
\frac{39,6°}{2} = 19,79°
$$

* **Passo 3: Calcular a largura (HFW)**: Calculamos a largura usando a tangente desse ângulo e a distância de 4 metros:
...
| --------------------- | ------- | ------- |
| **Ângulo horizontal da lente (HFOV)** | $2 \times \arctan( \text{Metade da Largura do Sensor} / \text{Distância Focal} )$ | Graus (°) |
| **Largura física da cena (HFW)** | $2 \times D \times \tan( \text{HFOV} / 2 )$ | Metros (m) |
